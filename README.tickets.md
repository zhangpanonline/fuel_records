...
1. **我先讲概念**：用大白话解释这个 Ticket 涉及的每个新概念（比如"什么是 ORM"、"FastAPI 路由怎么工作"）
2. **逐行写代码**：我写一行，解释一行，你跟着写一行
3. **你提问我回答**：任何一行不理解，随时打断我问
4. **做总结**：Ticket 完成后，我会用大白话总结你刚学到的所有知识点
5. **确认后才前进**：每个 Ticket 完成后，我会明确问你——"这个 Ticket 的所有内容你都完全理解了吗？"
...
- [ ] **2.1.3 级联油耗重算工具函数**
  - 封装 `recalculate_consumption(vehicle_id, after_date)` 函数
  - 被修改/删除影响的所有后继记录统一重算
  - **依赖**：2.1.1
  - **难度**：★★★
