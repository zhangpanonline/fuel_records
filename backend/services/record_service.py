...
def create_record(db: Session, record_in: FuelRecordCreate, user_id: int) -> FuelRecord:
    ...
    - user_id: 当前登录用户的 ID
    """
    # 1. 查询该用户的上一条记录（按 record_date 倒序）
    last_record = (
        db.query(FuelRecord)
        .filter(FuelRecord.user_id == user_id)
...
def get_records(
    ...
    """获取加油记录列表（分页，按时间倒序），按 user_id 过滤"""
...
def recalculate_consumption(db: Session, from_record_date: datetime, user_id: int) -> None:
    """级联重算：从指定日期开始，重新计算该用户所有后继记录的油耗
    ...
    previous = (
        db.query(FuelRecord)
        .filter(FuelRecord.user_id == user_id, FuelRecord.record_date < from_record_date)
        .order_by(desc(FuelRecord.record_date))
        .first()
    )
...
def update_record(
    ...
    db.commit()
    recalculate_consumption(db, db_record.record_date, user_id)
    db.refresh(db_record)
...
def delete_record(db: Session, record_id: int, user_id: int) -> FuelRecord:
    ...
    recalculate_consumption(db, record_date, user_id)
